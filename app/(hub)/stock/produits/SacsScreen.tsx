...
import { chargerSacsInventaires, chargerVentesSumupLignes, enregistrerInventaireSacs, synchroniserVentesSumup } from './actions';
import {
  PRODUITS_SACS,
  calculerARamenerSacs,
  resoudreVentesSumupSacs,
  type SacInventaire,
  type SacMappingSumup,
  type SacStock,
...
/** Réplique SacsScreen.tsx de l'app — mêmes principes que ChaussuresScreen, groupé par
 * produit > couleur. */
export function SacsScreen({
  ...
  popUpNom: string;
  stock: SacStock[];
  mapping: SacMappingSumup[];
}) {
  const [inventaires, setInventaires] = useState<SacInventaire[] | null>(null);
  ...
  const recharger = () => {
    chargerSacsInventaires(popUpId).then(setInventaires);
  ...
  const ventes = useMemo(() => resoudreVentesSumupSacs(ventesLignes ?? [], mapping), [ventesLignes, mapping]);
  const avecARamener = useMemo(() => calculerARamenerSacs(stock, inventaires ?? [], ventes), [stock, inventaires, ventes]);
  const parProduit = useMemo(() => {
    const map = new Map<string, SacStock[]>();
    for (const item of stock) map.set(item.produit, [...(map.get(item.produit) ?? []), item]);
    return map;
  }, [stock]);
  ...
  const validerInventaire = () => {
    const lignes = stock.map((item) => ({ produit: item.produit, couleur: item.couleur, quantite_comptee: Number(comptage[item.id]) || 0 }));
    if (!confirm("Enregistre ce comptage — ça recalcule directement ce qu'il faut ramener.")) return;
    demarrer(async () => {
      await enregistrerInventaireSacs(lignes, popUpId);
      setComptage({});
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Sacs &amp; pochettes</h2>
      ...
      {chargement ? (
        ...
        <>
          <p className="mb-3 text-xs text-slate-400">
            Stock estimé en temps réel, par produit et par couleur — dernier comptage moins les ventes SumUp survenues depuis.
          </p>
        ...
        <>
          <p className="mb-3 text-xs text-slate-400">Compte ce qu&apos;il reste vraiment, par produit et par couleur, puis valide.</p>
          {PRODUITS_SACS.map((produit) => (
            <div key={produit} className="mb-4 rounded-2xl border border-slate-200 bg-white p-4">
              <p className="mb-3 text-base font-bold text-slate-900">{produit}</p>
              <div className="flex flex-wrap gap-3">
